<style>body{ margin-bottom: 300px; }</style>
...
$result = $pdo->query("SELECT DISTINCT service FROM employes");  // fonctionne aussi ici avec GROUP BY, mais ce dernier est plutôt utilisé avec les fonctions qui assemblent des éléments comme SUM(), COUNT(), MIN(), MAX() et AVG()
...
foreach($donnees as $employe) {
	//debug($employe);  // $employe correspond aux sous arrays qui représente 1 employé à chaque tour de boucle
...
$resultat->bindParam(':nom', $nom);  // bindParam() reçoit exclusivement une variable vers laquelle pointe le marqueur. On ne peut pas y mettre une valeur directement, sinon il faut utiliser bindValue() à la place de bindParam().
...
/*
   prepare() permet de préparer le requête mais ne l'exécute pas.
   ...
   Valeurs de retour :
	...
	Les requêtes préparées sont préconisées si vous exécutez plusieurs fois la même requête et ainsi éviter de refaire le cycle complet analyse/inteprétation/exécution réalisé par le SGBD (on ne refait que le execute).
...
$resultat->execute(array(':nom' => 'chevel', ':prenom' => 'daniel'));  // on peut associer directement dans les () de execute() les marqueurs de la requête SQL à leur valeur. Notez qu'il est possible de ne pas mettre les ":" avant le nom des marqueurs dans cet array. 
