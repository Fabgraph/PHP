...
if (!empty($_POST)) {  // si $_POST est rempli c'est que le formulaire a été soumis
	...
	function validateDate($date, $format = 'd-m-Y') {   // $format = 'd-m-Y' permet de donner une valeur par défaut au paramètre $format si on ne lui passe pas d'argument lors de l'appel de la fonction
		...
		if ($d && $d->format($format) == $date) {  // si $d n'est pas false (voir ci-dessus) et que l'objet date $d est bien égal à la date $date, c'est qu'il n'y a pas eu d'extrapolation sur la date : exemple de 32/01/2015 qui devient 01/02/2015. Dans ce cas la date est validé : on retourne true.
			return true;
		} else {
			return false;
		}
