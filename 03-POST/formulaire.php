...
//------------------------
// $_POST est une superglobale qui permet de récupérer les données saisies dans le formulaire.

// $_POST étant une superglobale, il s'agit d'un ARRAY, et il est disponible dans tous les contextes du script, y compris au sein des fonctions (sans faire glogal $_POST).
...
if (!empty($_POST)) {  // si $_POST n'est pas vide, c'est qu'il est rempli par des données reçues de l'internaute
    ...
    // dans un futur proche on vérifira ici les données reçues avant de les traiter

    $message = '<p>Prénom : ' . $_POST['prenom'] . '</p>';
    $message .= '<p>Description : ' . $_POST['description'] . '</p>'; // .= veut dire j'ajoute.
