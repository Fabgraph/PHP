...
$resultat = executeRequete("SELECT DISTINCT categorie FROM produit"); // DISTINCT pour déboulonner les catégories existantes. $resultat est un objet PDOstatement
...
$contenu_gauche .= '<div class="list-group">';
    ...
    while ($cat = $resultat->fetch(PDO::FETCH_ASSOC)) {
        ...
        $contenu_gauche .= '<a href="?categorie='. $cat['categorie'] .'" class="list-group-item">'. $cat['categorie'] .'</a>';  // on met l'array $cat['categorie'] à la place de "tous" pour récupérer à chaque tour de boucle chacune des catégories présentent dans cet array (voir le debug ci-dessus)
    }
...
if (isset($_GET['categorie']) && $_GET['categorie'] != 'tous') {
    // si existe "categorie" dans $_GET (donc dans l'url), c'est qu'ona cliqué sur une catégorie. De plus, si elle est différente de "tous" c'est qu'on a choisi une catégorie en particulier (robe, pul...). On sélectionne donc tous les produits de CETTE catégorie :
...
while ($produit = $donnees->fetch(PDO::FETCH_ASSOC)) {
    ...
    $contenu_droite .= '</div>'; // .col-sm-4 md-4
}
