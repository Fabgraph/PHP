       ...
       // debug($ligne);
		$contenu .= '<tr>';
			...
			foreach($ligne as $indice => $info) {
                if ($indice == 'photo') {
                    $contenu .= '<td><img src="../'  . $info . '" width="90" alt="'. $ligne['titre'] .'"></td>';
                } else {
                    $contenu .= '<td>' . $info . '</td>';
                }
            }

            $contenu .= '<td><a href="?id_produit='. $ligne['id_produit'] .'"  onclick="return(confirm(\'Etes-vous certain de vouloir supprimer ce produit ?\'))">supprimer</a></td>';  // $ligne['id_produit'] contient l'id de chaque produit à chauqe tour de boucle while : ainsi le lien est dynamique, l'id passé en GET change selon le produit sur lequel je clique
