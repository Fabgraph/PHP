...
// 1- On vérifie que le rpoduit demandé existe bien (un produit en favori a pu être supprimé de la bdd...) :
...
// Exercice : afficher 2 produits (photo et titre) aléatoirement appartenant à la catégorie du produit affiché dans la fiche_produit.php. La photo est cliquable et amène à la fiche du produit. Utilisez la variable $suggestion pour  fficher le contenu. Complément : pour sélectionner aléatoirement des produits, vous utilisez la fonction ORDER BY RAND() dans la requête SQL.
    // 1- Affichage des produits selon la catégorie choisie :
        $resultat = executeRequete("SELECT id_produit, titre, photo FROM produit WHERE categorie = :categorie ORDER BY RAND() LIMIT 2", array(':categorie' => $categorie)); // $categorie contient la categorie du produit actuellement affiché dans la fiche_produit.php. En SQL, la fonction LIMIT permet de limter le nombre de résultats de la requête au nombre spécifié. ORDER BY RAND() fait un tri aléatoire des résultats. Attention à l'ordre de ces fonctions : TOUJOURS WHERE puis ORDER BY puis LIMIT.
